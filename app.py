...
# 任务内存索引：rid -> {q:Queue, dir:str, done:bool, summary:list, ts, role, track:list}
...
def llm_chat(messages: List[Dict[str,str]], temperature: float=0.2, max_tokens:int=1024) -> str:
    ...
    重点：base_url + /v1/chat/completions，不要出现 /v1/v1。
    """
...
PROMPT_SYS = (
"你是资深猎头助理。"
"请基于候选人简历文本，提取结构化字段并做岗位匹配。"
"输出 JSON，不要解释。"
"字段：name, current_company, current_title, email, location, age_estimate, tags(list), "
"education(list: {school, major, degree, start, end}), "
"experiences(list: {company, title, start, end, one_line}), "
"remark(教育/工作时间线概述，用中文，100字内), "
...
def process_resume(path:str, role:str, track:str, note:str)->Dict[str,Any]:
    ...
    msg = build_messages(role,track,note,text[:12000])  # 截断保护
...
def write_excel(rows: List[Dict[str,Any]], xlsx_path:str):
    ...
    for r in rows:
        ws.append([
            ...
            r.get("age_estimate",""),
            r.get("location",""),
            r.get("fit_summary",""),  # 由模型可选返回，若无则空
            r.get("risks",""),
            ", ".join(r.get("tags",[]) or []),
            r.get("remark",""),
        ])
...
def resume(rid):
    # 断点续跑 = 再触发一次“runner”，会自动跳过已处理/去重
    run = ensure_run(rid)
    if run.get("done"):
        return redirect(url_for("events", rid=rid))
    # 只需往队列里丢一条提示让用户知道在跑；真正逻辑仍在原 runner 内部
